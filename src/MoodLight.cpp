...
void MoodLight::update(uint32_t nowMs) {
  ...
  if (isHolding) {
    ...
    const uint16_t holdMs = MOODS[moodIndex].holdMs;
    if ((uint32_t)(nowMs - holdStartMs) < holdMs) return;
    advanceToNextMood(nowMs);
    return;
  }
...
void MoodLight::writeCommonAnodePwm(const Rgb8& c){ analogWrite(pinR,255-c.r); analogWrite(pinG,255-c.g); analogWrite(pinB,255-c.b); }
uint8_t MoodLight::scaleAndClamp(uint8_t v,uint8_t s){ uint16_t r=(uint16_t)v*(uint16_t)s/255u; return (r>255u)?255u:(uint8_t)r; }
uint8_t MoodLight::triangleWave(uint32_t t,uint16_t p){ if(!p)return 255; uint32_t m=t%p,h=p/2u; if(m<h) return (uint8_t)((m*255u)/(h?h:1u)); uint32_t d=m-h; return (uint8_t)(255u-(d*255u)/(h?h:1u)); }
uint8_t MoodLight::pulseWave(uint32_t t,uint16_t p,uint8_t duty){ if(!p)return 255; uint32_t m=t%p,thr=((uint32_t)p*duty)>>8; return (m<thr)?255:0; }
uint8_t MoodLight::heartbeatWave(uint32_t t,uint16_t p){ if(!p)return 255; uint32_t m=t%p; uint32_t p1_on=(p*0)/100,p1_off=(p*10)/100,p2_on=(p*14)/100,p2_off=(p*24)/100; if((m>=p1_on&&m<p1_off)||(m>=p2_on&&m<p2_off))return 255; if(m>=p2_off&&m<(p2_off+(p*8)/100)){ uint32_t d=m-p2_off,len=(p*8)/100; return (uint8_t)(200u-(d*200u)/(len?len:1u)); } return 0; }
uint8_t MoodLight::flickerJitter(){ uint16_t x=(uint16_t)(lfsr&0xFFFFu); uint16_t bit=((x>>0)^(x>>2)^(x>>3)^(x>>5))&1u; x=(uint16_t)((x>>1)|(bit<<15)); lfsr=(lfsr&0xFFFF0000u)|x; return (uint8_t)(x>>8); }
const char* MoodLight::patternName(PatternType p){ switch(p){ case PatternType::Static:return "Static"; case PatternType::Breathe:return "Breathe"; case PatternType::Pulse:return "Pulse"; case PatternType::Heartbeat:return "Heartbeat"; case PatternType::Flicker:return "Flicker"; case PatternType::BlinkAlt:return "BlinkAlt"; default:return "Unknown"; } }
